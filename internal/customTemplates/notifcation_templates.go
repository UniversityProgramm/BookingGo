...
var NotificationTemplates = map[string]*template.Template{
	...
	"booking_completed": template.Must(template.New("booking_completed").Parse("{{.UserName}}, статус вашей записи был изменен на «Завершена»")),
	"booking_canceled":  template.Must(template.New("booking_canceled").Parse("Запись на {{.BookingDate}} с проблемой {{.Message}} отменена.")),
	"booking_auth":      template.Must(template.New("booking_auth").Parse("Вы вошли в аккаунт с IP: {{.IP}}")),
}
